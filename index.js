...
//key=${id} --> Element Identifier is been missing beside id  
const htmlTaskContent = ({ id, title, description, type, url }) => `
 <div class="col-md-6 col-lg-4 mt-3" id=${id} key=${id}>
 <div class="card shadow-sm task__card">

 <div class="card-header d-flex justify-content-end task__card__header">
  <button type="button" class="btn btn-outline-primary mr-2.5" name=${id} onclick="editTask.apply(this, arguments)"> 
...
//Modal Body after Clicking open task (Button)
const htmlModalContent = ({ id, title, description, url}) => {
   const date = new Date(parseInt(id));
    ...
    ${
     // url &&
    //  `<img width="100" src=${url} alt="Card Image" class="img-fuild placeholder__image mb-3" /> `
    url
    ? `<img width='100%' src=${url} alt='Card Image' class='card-img-top md-3 rounded-lg' />`
    : `<img width='100%' src="https://tse1.mm.bing.net/th?id=OIP.F00dCf4bXxX0J-qEEf4qIQHaD6&pid=Api&rs=1&c=1&qlt=95&w=223&h=117" alt='Card Image' class='card-img-top md-3 rounded-lg' />`
   }
...
//where we convert JSon > str i.e. for our local storage
...
// where we convert str > json i.e. for rendering the cards on screen 
//Load Initial Data
...
const handleSubmit = (event) => {
 //  console.log("event trigerred");
      ...
      if(input.taskTitle==="" || input.type==="" || input.description==="") {
         return alert ("Please fill in all the necessary fields :-)"); 
      } 
...
const editTask = (e) => {
  ...
  // taskTitle = parentNode.childNodes[3].childNodes[7];
